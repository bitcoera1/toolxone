...
function calculateROI() {
    ...
    result.innerHTML = `
        <div class="result-line">
            ...
            <strong>${investment.toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})}</strong>
        </div>

        <div class="result-line">
            <span>Final Return</span>
            <strong>${returnAmount.toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})}</strong>
        </div>

        <div class="result-line">
            <span>Profit / Loss</span>
            <strong>${profit.toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})}</strong>
        </div>
    ...
    const investmentPercent = returnAmount > 0
        ? Math.min((investment / returnAmount) * 100, 100)
        : 100;

    const profitPercent = profit > 0 && returnAmount > 0
        ? Math.min((profit / returnAmount) * 100, 100)
        : 0;
